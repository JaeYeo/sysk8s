...
type ClusterServiceClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ClusterServiceClass, err error)
	// Get retrieves the ClusterServiceClass from the index for a given name.
	Get(name string) (*v1.ClusterServiceClass, error)
...
// List lists all ClusterServiceClasses in the indexer.
func (s *clusterServiceClassLister) List(selector labels.Selector) (ret []*v1.ClusterServiceClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterServiceClass))
	})
	return ret, err
}

// Get retrieves the ClusterServiceClass from the index for a given name.
func (s *clusterServiceClassLister) Get(name string) (*v1.ClusterServiceClass, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterserviceclass"), name)
	}
	return obj.(*v1.ClusterServiceClass), nil
}

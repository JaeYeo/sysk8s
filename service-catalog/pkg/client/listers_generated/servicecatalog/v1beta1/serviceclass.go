...
type ServiceClassLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ServiceClass, err error)
...
// List lists all ServiceClasses in the indexer.
func (s *serviceClassLister) List(selector labels.Selector) (ret []*v1.ServiceClass, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServiceClass))
	})
	return ret, err
}
...
type ServiceClassNamespaceLister interface {
	...
	Get(name string) (*v1.ServiceClass, error)
...
// List lists all ServiceClasses in the indexer for a given namespace.
func (s serviceClassNamespaceLister) List(selector labels.Selector) (ret []*v1.ServiceClass, err error) {
...
// Get retrieves the ServiceClass from the indexer for a given namespace and name.
func (s serviceClassNamespaceLister) Get(name string) (*v1.ServiceClass, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("serviceclass"), name)
	}
	return obj.(*v1.ServiceClass), nil
}

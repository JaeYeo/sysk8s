...
type ClusterServicePlanLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ClusterServicePlan, err error)
	// Get retrieves the ClusterServicePlan from the index for a given name.
	Get(name string) (*v1.ClusterServicePlan, error)
...
// List lists all ClusterServicePlans in the indexer.
func (s *clusterServicePlanLister) List(selector labels.Selector) (ret []*v1.ClusterServicePlan, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterServicePlan))
	})
	return ret, err
}

// Get retrieves the ClusterServicePlan from the index for a given name.
func (s *clusterServicePlanLister) Get(name string) (*v1.ClusterServicePlan, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterserviceplan"), name)
	}
	return obj.(*v1.ClusterServicePlan), nil
}

...
type ClusterServiceBrokerLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ClusterServiceBroker, err error)
	// Get retrieves the ClusterServiceBroker from the index for a given name.
	Get(name string) (*v1.ClusterServiceBroker, error)
...
// List lists all ClusterServiceBrokers in the indexer.
func (s *clusterServiceBrokerLister) List(selector labels.Selector) (ret []*v1.ClusterServiceBroker, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterServiceBroker))
	})
	return ret, err
}

// Get retrieves the ClusterServiceBroker from the index for a given name.
func (s *clusterServiceBrokerLister) Get(name string) (*v1.ClusterServiceBroker, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterservicebroker"), name)
	}
	return obj.(*v1.ClusterServiceBroker), nil
}

...
type ServicePlanLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ServicePlan, err error)
...
// List lists all ServicePlans in the indexer.
func (s *servicePlanLister) List(selector labels.Selector) (ret []*v1.ServicePlan, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServicePlan))
	})
	return ret, err
}
...
type ServicePlanNamespaceLister interface {
	...
	Get(name string) (*v1.ServicePlan, error)
...
// List lists all ServicePlans in the indexer for a given namespace.
func (s servicePlanNamespaceLister) List(selector labels.Selector) (ret []*v1.ServicePlan, err error) {
...
// Get retrieves the ServicePlan from the indexer for a given namespace and name.
func (s servicePlanNamespaceLister) Get(name string) (*v1.ServicePlan, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("serviceplan"), name)
	}
	return obj.(*v1.ServicePlan), nil
}

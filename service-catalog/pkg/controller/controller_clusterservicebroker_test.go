...
func TestShouldReconcileClusterServiceBroker(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "no status",
			broker: func() *v1.ClusterServiceBroker {
		...
		},
		{
			...
			broker: func() *v1.ClusterServiceBroker {
				broker := getTestClusterServiceBrokerWithStatus(v1.ConditionTrue)
				...
				broker.Status = v1.ClusterServiceBrokerStatus{
					CommonServiceBrokerStatus: v1.CommonServiceBrokerStatus{
						Conditions: []v1.ServiceBrokerCondition{
							{
								Type:   v1.ServiceBrokerConditionType("NotARealCondition"),
		...
		},
		{
			name: "not ready",
			broker: func() *v1.ClusterServiceBroker {
				broker := getTestClusterServiceBrokerWithStatus(v1.ConditionFalse)
				broker.Spec.RelistDuration = &metav1.Duration{Duration: 3 * time.Minute}
				return broker
			}(),
			now:       time.Now(),
		...
		},
		{
			...
			broker: func() *v1.ClusterServiceBroker {
				...
				broker := getTestClusterServiceBrokerWithStatusAndTime(v1.ConditionTrue, lastTransitionTime, lastRelistTime)
				broker.Spec.RelistDuration = &metav1.Duration{Duration: 3 * time.Minute}
				return broker
			}(),
			now:       time.Now(),
		...
		},
		{
			...
			broker: func() *v1.ClusterServiceBroker {
				...
				broker := getTestClusterServiceBrokerWithStatusAndTime(v1.ConditionTrue, t, t)
				broker.Spec.RelistBehavior = v1.ServiceBrokerRelistBehaviorDuration
		...
		},
		{
			...
			broker: func() *v1.ClusterServiceBroker {
				broker := getTestClusterServiceBrokerWithStatus(v1.ConditionTrue)
				broker.Spec.RelistBehavior = v1.ServiceBrokerRelistBehaviorManual
				return broker
			}(),
			now:       time.Now(),
			reconcile: false,
...
func TestReconcileClusterServiceBrokerExistingServiceClassAndServicePlan(t *testing.T) {
	...
	fakeCatalogClient.AddReactor("list", "clusterserviceclasses", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		return true, &v1.ClusterServiceClassList{
			Items: []v1.ClusterServiceClass{
	...
	updateObject, ok := updatedClusterServiceBroker.(*v1.ClusterServiceBroker)
	if !ok {
		t.Fatalf("couldn't convert to *v1.ClusterServiceBroker")
	}
...
func TestReconcileClusterServiceBrokerRemovedAndRestoredClusterServiceClass(t *testing.T) {
	...
	fakeCatalogClient.AddReactor("list", "clusterserviceplans", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		return true, &v1.ClusterServicePlanList{
			Items: []v1.ClusterServicePlan{
...
func TestReconcileClusterServiceBrokerExistingClusterServicePlanDifferentClass(t *testing.T) {
	...
	testClusterServicePlan.Spec.ClusterServiceClassRef = v1.ClusterObjectReference{
...
func getClusterServiceBrokerReactor(broker *v1.ClusterServiceBroker) (string, string, clientgotesting.ReactionFunc) {
	return "get", "clusterservicebrokers", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		return true, broker, nil
	}
}

func listClusterServiceClassesReactor(classes []v1.ClusterServiceClass) (string, string, clientgotesting.ReactionFunc) {
...
func listClusterServicePlansReactor(plans []v1.ClusterServicePlan) (string, string, clientgotesting.ReactionFunc) {
...
func TestReconcileClusterServiceBrokerDelete(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			fakeCatalogClient.AddReactor(listClusterServiceClassesReactor([]v1.ClusterServiceClass{*testClusterServiceClass}))
			fakeCatalogClient.AddReactor(listClusterServicePlansReactor([]v1.ClusterServicePlan{*testClusterServicePlan}))
			...
			assertList(t, catalogActions[0], &v1.ClusterServiceClass{}, listRestrictions)
			assertList(t, catalogActions[1], &v1.ClusterServicePlan{}, listRestrictions)
...
func testReconcileClusterServiceBrokerWithAuth(t *testing.T, authInfo *v1.ClusterServiceBrokerAuthInfo, secret *corev1.Secret, shouldSucceed bool) {
	fakeKubeClient, fakeCatalogClient, fakeClusterServiceBrokerClient, testController, _ := newTestController(t, fakeosb.FakeClientConfiguration{})
...
func TestReconcileClusterServiceBrokerWithReconcileError(t *testing.T) {
	...
	createdSC, ok := createSCAction.GetObject().(*v1.ClusterServiceClass)
	if !ok {
...
func TestReconcileClusterServiceBrokerSuccessOnFinalRetry(t *testing.T) {
	...
	assertList(t, actions[1], &v1.ClusterServiceClass{}, listRestrictions)
	assertList(t, actions[2], &v1.ClusterServicePlan{}, listRestrictions)
...
func TestReconcileClusterServiceBrokerFailureOnFinalRetry(t *testing.T) {
	...
	updatedClusterServiceBroker = assertUpdateStatus(t, actions[1], broker)
	assertClusterServiceBrokerCondition(t, updatedClusterServiceBroker, v1.ServiceBrokerConditionFailed, v1.ConditionTrue)
	assertClusterServiceBrokerOperationStartTimeSet(t, updatedClusterServiceBroker, false)
...
func TestUpdateServiceBrokerCondition(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			input:                 getTestClusterServiceBrokerWithStatus(v1.ConditionFalse),
			status:                v1.ConditionFalse,
		...
		{
			name:                  "ready -> ready",
			input:                 getTestClusterServiceBrokerWithStatus(v1.ConditionTrue),
			status:                v1.ConditionTrue,
			transitionTimeChanged: false,
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := testController.updateClusterServiceBrokerCondition(tc.input, v1.ServiceBrokerConditionReady, tc.status, tc.reason, tc.message)
			if err != nil {
			...
			updateActionObject, ok := updatedClusterServiceBroker.(*v1.ClusterServiceBroker)
			if !ok {
...
func TestReconcileClusterServicePlanFromClusterServiceBrokerCatalog(t *testing.T) {
	updatedPlan := func() *v1.ClusterServicePlan {
	...
	cases := []struct {
		name                    string
		newServicePlan          *v1.ClusterServicePlan
		existingServicePlan     *v1.ClusterServicePlan
		listerServicePlan       *v1.ClusterServicePlan
		shouldError             bool
		errText                 *string
		...
		},
		{
			...
			listerServicePlan: func() *v1.ClusterServicePlan {
				p := getTestClusterServicePlan()
...
func reconcileClusterServiceBroker(t *testing.T, testController *controller, broker *v1.ClusterServiceBroker) error {
...
func TestIsServiceCatalogManagedResource(t *testing.T) {
	testcases := []struct {
		...
		{"unmanaged service class", &v1.ServiceClass{}, false},
		{"unmanaged service plan", &v1.ServicePlan{}, false},
		{"managed service class", &v1.ServiceClass{ObjectMeta: metav1.ObjectMeta{OwnerReferences: []metav1.OwnerReference{
			{Controller: truePtr(), APIVersion: v1.SchemeGroupVersion.String()}}}}, true},
		{"managed service plan", &v1.ServicePlan{ObjectMeta: metav1.ObjectMeta{OwnerReferences: []metav1.OwnerReference{
			{Controller: truePtr(), APIVersion: v1.SchemeGroupVersion.String()}}}}, true},
	}
...
func TestMarkAsServiceCatalogManagedResource(t *testing.T) {
	testcases := []struct {
		...
		{"service class", &v1.ServiceClass{}},
		{"service plan", &v1.ServicePlan{}},
	}
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			...
			wantAPIVersion := v1.SchemeGroupVersion.String()

...
func (c *ControllerHealth) runAddonsConfigurationControllerCycle(client client.Client, lg *logrus.Entry) error {
	...
	lg.Infof("[liveness-probe] Context: %v", ctx)
	lg.Infof("[liveness-probe] AddonsConfiguration: %v", addonsConfiguration)
	err := client.Create(ctx, addonsConfiguration)
	if err != nil {
		if apierrors.IsAlreadyExists(err) {
			lg.Infof("[liveness-probe] Liveness probe addonsConfiguration already exists: %s", err)
		} else {
			lg.Errorf("[liveness-probe] Failed to create liveness probe addonsConfiguration: %v", err)
		}
		return err
	}
